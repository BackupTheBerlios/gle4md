...
program harmonic
  ...
  !   even if that makes not much sense. again, this is hust a small
  !   test code
  ...
  integer argc, seed, nstep, stride, ndof, nchains, nhmts
  character *256 fname, wfile, prefix
  namelist /inp/ seed, wfile, dt, temp, nstep, stride, traj, ndof, nchains, nhmts, nhw, wnw, glew
  ...
  enddo
  ! init coordinates (requires inverting wm ONLY WORKS IF WM IS SQRT(HESSIAN), NOT IF DONW WITH CHOLESKY!!!)
#ifdef USELIBS
  do i=1,ndof
  ...
  if (traj) open(102,file='traj-p.out') 
  if (traj) open(103,file='traj-q.out') 
